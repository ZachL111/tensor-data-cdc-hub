...
`tensor-data-cdc-hub` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies cdc behavior through safe and unsafe fixtures, with remediation hints and local-only command execution.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 3.
